...
public class Context {
    ...
    public void reg(Class cl) {
        ...
        if (constructors.length > 1) {
            throw new IllegalStateException("Class have multiplay constructor" + cl.getCanonicalName());
        }
        Constructor con = constructors[0];
        List<Object> args = new ArrayList<>();
        for (Class arg : con.getParameterTypes()) {
            if (!els.containsKey(arg.getCanonicalName())) {
                throw new IllegalStateException("Object doesn't found in context : 0 " + arg.getCanonicalName());
            }
